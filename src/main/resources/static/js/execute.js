/**
 * Trading Bot — Custom Execution View (execute.html & execute-screener.html forms)
 */
...
async function executeCustom() {
    ...
    try {
        ...
        const res = await API.executeCustomStrategy(body);
...
async function loadCustomResults() {
    ...
    if (!container) return;
    try {
        const results = await API.getRecentCustomResults();
        container.innerHTML = '';
