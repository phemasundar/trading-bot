...
let _calEvents = {};      // date-string → array of event objects
let _calCurrentDate = new Date();  // tracks which month is displayed
...
function renderCalendar() {
    ...
    // Remove previous day cells (keep 7 header cells)
    while (grid.children.length > 7) {
        grid.removeChild(grid.lastChild);
    }

    // First day of this month and total days
...
function onDayClick(dateStr, date) {
    ...
    renderCalendar(); // re-render to update selection styling
