...
public class SampleTestNG {
    public static void main(String[] args) {
        ...
        System.out.println("1. Log in via your browser. \nhttps://api.schwabapi.com/v1/oauth/authorize?client_id=" + TestConfig.getInstance().appKey() + "&redirect_uri=https://127.0.0.1");
        System.out.println("2. Copy the FULL URL of the page you are redirected to (the one starting with https://127.0.0.1).");
    ...
    public void getOptionChainData() throws IOException {
        Securities securities = JavaUtils.convertYamlToPojo(Files.readString(FilePaths.securitiesConfig), Securities.class);
...
//        System.out.println("Response: \n" + response.asPrettyString());
        ...
        OptionChainResponse optionChainResponse = JavaUtils.convertJsonToPojo(response.asString(), OptionChainResponse.class);
        System.out.println("Current Maket Price: " + optionChainResponse.getUnderlyingPrice());
        return optionChainResponse;
    }

    /**
     * Finds Put Credit Spreads based on 12% profit/loss ratio, $1000 max risk, and 0.6 delta.
     *
    ...
    public void findValidPutCreditSpreads(Map<String, List<OptionChainResponse.OptionData>> putMap, double maxDelta, double maxLossLimit, int returnOnRiskPercentage) {
        ...
        // In a Put Credit Spread, the Short Put is the one with the HIGHER strike price.
        for (int i = 0; i < sortedStrikes.size(); i++) {
            double shortStrikePrice = sortedStrikes.get(i);

            // 3. Retrieve the OptionData for the Short Put. We assume the list contains at least one entry.
            ...
            // The Long Put must have a LOWER strike price than the Short Put (indices 0 to i-1).
            for (int j = 0; j < i; j++) {
                ...
                // 7. Ignore invalid data: If netCredit is zero or negative, it's not a credit spread.
                ...
                if (maxLoss > maxLossLimit) continue;
