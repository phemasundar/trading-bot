...
- **Technical Indicators**: RSI (with customizable conditions including custom min/max ranges), Bollinger Bands, Average True Range (ATR), and Volume analysis using ta4j library
- **Fundamental Filters**: Dynamic evaluation of fundamental data metrics like Market Cap using mathematical expressions (e.g., `"market_cap_b > 100"`). Centralized `getMarketCapB()` helper method on `QuotesResponse.QuoteData` calculates market cap in billions dynamically from underlying quote and fundamental data.
- **Telegram Notifications**: Receive trade alerts directly to your Telegram
- **Strategy History & Similar Trades**: Asynchronously records historical trade executions into Supabase (`historical_trades` table) using SHA-256 deterministic trade hashes for duplicate prevention. Each trade row on the dashboard includes a **History button (🕒)** that opens an interactive modal listing matching historical trades filtered dynamically by leg structures, strike offsets, and ticker symbol.
- **Interactive UI Dashboard**: Execute custom strategy instances, explore strategy configurations with user-defined technical screener aliases, and view data-rich technical screening results with customizable indicator tables. Screener results tables conveniently display Market Cap (always normalized in Billions, e.g. `$1040.00B`) as the 3rd column directly following Current Price, complete with full interactive column sorting. Integrated Charles Schwab API provides live Market Hours status directly on the dashboard. Centralized rate limit (429) pause and retry execution powered by `SchwabApiExecutor` across all market data endpoints (Option Chain and Quotes API), with parallel multithreaded pre-warming of option chains (for non-filtered strategies and screened survivors) and quotes (for all strategy and screener securities before technical indicator pre-calculation), alongside on-demand fallback fetch mechanisms in `TechnicalScreener`. Options trade tables on both the local dashboard and the static GitHub Pages dashboard include a live **Today's Performance** column (where applicable), intelligent table sorting supporting annualized CAGR yields, and a dual-metric ROR display formatting the raw return percentage alongside its annualized CAGR in brackets (e.g., `12.5% (150.0% CAGR)`) with dynamic client-side fallback calculation for older strategy execution results stored in Supabase. When a trade row is expanded, the dashboard displays any **Technical Indicators** calculated during the strategy execution (such as RSI, Bollinger Bands, Moving Averages, and Volume metrics) in a dedicated block next to the trade details, option chain data, and volatility context.
